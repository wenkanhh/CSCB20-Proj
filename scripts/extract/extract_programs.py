#!/usr/bin/env python3
"""
Program extraction and inspection utility

This module consolidates:
- extract_programs.py: Extract program data from programs.txt and generate CSVs
- inspect_empty_programs.py: Inspect programs.csv for empty/missing program codes
"""
...
# ============================================================================
# PROGRAM EXTRACTION FUNCTIONS
# ============================================================================

def extract_programs():
    """
    Extract program data from programs.txt and generate three CSV files:
    - programs.csv: Program metadata
    - program_requirement.csv: Requirement groups per program
    - program_requirement_courses.csv: Courses linked to requirement groups
    """
    ...
    return progs_df, reqs_df, courses_df
...
def inspect_empty_programs():
    """
    Inspect programs.csv for empty or missing program codes.
    
    Prints:
    - Total count of programs with empty program_code
    - First 50 programs with empty codes (showing code and name)
    """
    print("\n" + "=" * 80)
    print("PROGRAM INSPECTION: EMPTY PROGRAM CODES")
    print("=" * 80 + "\n")
    
    p = pd.read_csv('programs.csv')
    empty = p[p['program_code'].isna() | (p['program_code'] == '')]
    
    print(f'Empty program code count: {len(empty)}\n')
    
    if len(empty) > 0:
        print("Programs with empty program_code:")
        print(empty[['program_code', 'program_name']].head(50).to_string(index=False))
    else:
        print("[OK] All programs have program_code populated")
...
def main():
    """Main entry point for program extraction and inspection."""
    print("\n" + "=" * 80)
    print("PROGRAM EXTRACTION AND INSPECTION UTILITY")
    print("=" * 80)
    
    print("\nAvailable functions:")
    print("  1. extract_programs       - Extract programs from data_raw/programs.txt")
    print("  2. inspect_empty_programs - Inspect for empty program codes")
    print("  3. Run extract, then inspect")
    print("  0. Exit")
